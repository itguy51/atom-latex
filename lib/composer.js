...
export default class Composer {
  ...
  initializeBuildState (filePath) {
    return new BuildState(this.resolveRootFilePath(filePath))
  }
  ...
  async build (shouldRebuild) {
    ...
    const state = this.initializeBuildState(filePath)

    if (this.rebuildCompleted && !this.rebuildCompleted.has(state.rootFilePath)) {
      shouldRebuild = true
      this.rebuildCompleted.add(state.rootFilePath)
    }
    ...
    // await latex.steps.run(['initialize', 'magic-parse'], state)
    const buildSteps = ['log-parse']
    if (path.extname(state.rootFilePath) === '.Rnw') {
      buildSteps.unshift('knitr')
      buildSteps.unshift('latexmk')
    } else {
      buildSteps.unshift('latexmk')
    }

    const jobs = state.jobnames.map(jobname => latex.steps.run(buildSteps, state, jobname))
  ...
  async buildJob (builder, state, jobname) {
    try {
      const statusCode = await builder.run(state, jobname)
      const result = builder.parseLogAndFdbFiles(state.projectPath, state.rootFilePath, jobname)
      ...
      if (statusCode > 0 || !result || !result.outputFilePath) {
        ...
        if (this.shouldMoveResult(builder, state.rootFilePath)) {
          this.moveResult(result, state.rootFilePath)
        }
        // Cache the output file path for sync
        this.outputLookup.set({ rootFilePath: state.rootFilePath, jobname }, result.outputFilePath)
  ...
  async sync () {
    ...
    const state = this.initializeBuildState(filePath)
    const builder = this.getBuilder(state.rootFilePath)
    if (!builder) return false

    const jobs = state.jobnames.map(jobname => this.syncJob(filePath, lineNumber, builder, state.rootFilePath, jobname))
  ...
  async clean () {
    ...
    const jobs = state.jobnames.map(jobname => this.cleanJob(builder, state.rootFilePath, jobname))
  ...
  getBuilder (filePath) {
    ...
    if (Object.getPrototypeOf(BuilderImpl).constructor.name === 'TexifyBuilder') {
      // -------------------------------------------------------------
