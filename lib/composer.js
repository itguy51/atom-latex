...
export default class Composer {
  ...
  async build (shouldRebuild) {
    ...
    const jobs = state.jobnames.map(jobname => this.buildJob(builder, state, jobname, shouldRebuild))
  ...
  async buildJob (builder, state, jobname, shouldRebuild) {
    try {
      const statusCode = await builder.run(state, state.rootFilePath, jobname, shouldRebuild)
